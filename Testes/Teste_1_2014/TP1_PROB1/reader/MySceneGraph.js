...
MySceneGraph.prototype.parsePrimitives= function(primitivesElem) 
{
	...
	for(var i = 0; i < primitives_nnodes; i++)
	{
		...
		//LAIGPROB1 - Start
		else if(primitiveChild.nodeName == 'diamond')
		{
			var diamond_slices = this.reader.getFloat(primitiveChild, 'slices');
			if(isNaN(diamond_slices))
			{
				return "Error at the primitive(diamond) element with id = " + id + ". The slices value is not a number.";
			}
			primitiveData = new DiamondData(id, diamond_slices);
		}
		//LAIGPROB1 - End
		else
		{
