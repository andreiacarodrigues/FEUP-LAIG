...
MySceneGraph.prototype.parsePrimitives= function(primitivesElem) 
{
	...
	for(var i = 0; i < primitives_nnodes; i++)
	{
		...
		//LAIGPROB3 - Start
		else if(primitiveChild.nodeName == 'square')
		{
			var square_texangle = this.reader.getFloat(primitiveChild, 'texangle');
			if(isNaN(square_texangle))
			{
				return "Error at the primitive(square) element with id = " + id + ". The texangle value is not a number.";
			}
			primitiveData = new SquareData(id, square_texangle);
		}
		//LAIGPROB3 - End
		else
		{
