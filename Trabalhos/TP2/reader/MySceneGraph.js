...
MySceneGraph.prototype.parseDSX= function(rootElement) {
	...
	if(root_nnodes != 10)
	{
		return "Root element does not have exactly ten children elements. Number of elements found: " + root_nnodes + ".";
	}
...
MySceneGraph.prototype.parseComponents= function(componentsElem) 
{
	...
	for(var i = 0; i < components_nnodes; i++)
	{
		...
		if (animationElems == null) {
		return "Error at the component element with id = " + id + ". The 'animation' child element is missing at the 'component' element.";
		}

		if (animationElems.length > 1) {
			return "Error at the component element with id = " + id + ". More than one 'animation' elements found. Quantity found: " + animationElems.length + ".";
		}
		else if(animationElems.length == 1)
		{
			var animationElem = animationElems[0];

			if(animationElem.parentNode.nodeName != 'component') {
				return "Error at the component element with id = " + id + ". The 'animation' child element does not have 'component' as its parent element. Parent element found: " + animationElem.parentNode.nodeName + ".";
			}
			...
			for(let i = 0; i < animation_nnodes; i++)
			{
				...
				{
					return "Error at the component element with id = " + id + ". Animation element has a child with an invalid tag: " + materialElem.nodeName + ".";			
				}
...
MySceneGraph.prototype.parseAnimations= function(animationsElem) 
{
	...
	for(var i = 0; i < animations_nnodes; i++)
	{
		...
		{
			return "Error at the animation element with id = " + id + ". The animation type is neither 'linear', nor 'circular'.";
		}
