...
MySceneGraph.prototype.parseDSX= function(rootElement) {
	...
	if(root_nnodes != 11)
	{
		return "Root element does not have exactly eleven children elements. Number of elements found: " + root_nnodes + ".";
	}

	//Parse the scene element
	var sceneElems = rootElement.getElementsByTagName('scene');

	var errorMsg = this.testUniqueElement(sceneElems,'scene','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}

	var rootChild = sceneElems[0];
	...
	var errorMsg = this.testUniqueElement(viewsElems,'views','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}

	rootChild = viewsElems[0];
	...
	//Parse the viewpoints element
	var viewpointsElems = rootElement.getElementsByTagName('viewpoints');

	var errorMsg = this.testUniqueElement(viewpointsElems,'viewpoints','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = viewpointsElems[0];
	var errorMsg = this.parseViewpoints(rootChild);
	if(errorMsg != null)
	{
		return "Error at the 'viewpoints' element. " + errorMsg;
	}

	//Parse the illumination element
	var illuminationElems = rootElement.getElementsByTagName('illumination');

	var errorMsg = this.testUniqueElement(illuminationElems,'illumination','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = illuminationElems[0];
	...
	var errorMsg = this.testUniqueElement(lightsElems,'lights','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = lightsElems[0];
	...
	var texturesElems = rootElement.getElementsByTagName('textures');

	var errorMsg = this.testUniqueElement(texturesElems,'textures','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = texturesElems[0];
	...
	var materialsElems = rootElement.getElementsByTagName('materials');
	materialsElems = this.filterElemsByParent(materialsElems, 'dsx'); //beacause the component element also has materials as children

	var errorMsg = this.testUniqueElement(materialsElems,'materials','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = materialsElems[0];
	...
	var errorMsg = this.testUniqueElement(transformationsElems,'transformations','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = transformationsElems[0];
	...
	//Parse the animations element
	var animationsElems = rootElement.getElementsByTagName('animations');

	var errorMsg = this.testUniqueElement(animationsElems,'animations','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = animationsElems[0];
	var errorMsg = this.parseAnimations(rootChild);
	if(errorMsg != null)
	{
		return "Error at the 'animations' element. " + errorMsg;
	}
	
	//Parse the primitives element
	var primitivesElems = rootElement.getElementsByTagName('primitives');

	var errorMsg = this.testUniqueElement(primitivesElems,'primitives','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = primitivesElems[0];
	...
	var errorMsg = this.testUniqueElement(componentsElems,'components','dsx');
	if(errorMsg != null)
	{
		return errorMsg;
	}
	
	rootChild = componentsElems[0];
...
/*
 * Filters all of the components that do not have a specific parent.
 */
MySceneGraph.prototype.filterElemsByParent=function(elems, parentName)
{
	var filteredElems = [];
...
MySceneGraph.prototype.testUniqueElement= function(elemsFound, elemName, parentName)
{
	...
	if (elemsFound.length != 1) {
		return "Either zero or more than one " + elemName + " elements found at the " + parentName + " element. Quantity found: " + elemsFound.length + ".";
	}
...
MySceneGraph.prototype.parsePrimitives= function(primitivesElem) 
{
	...
	for(var i = 0; i < primitives_nnodes; i++)
	{
		...
		{
			var plane_dimX = this.reader.getFloat(primitiveChild, 'dimX');
			if(isNaN(plane_dimX))
			{
				return "Error at the primitive(plane) element with id = " + id + ". The dimX value is not a number.";
			}
			var plane_dimY = this.reader.getFloat(primitiveChild, 'dimY');
			if(isNaN(plane_dimY))
			{
				return "Error at the primitive(plane) element with id = " + id + ". The dimY value is not a number.";
			}
			var plane_partsX = this.reader.getInteger(primitiveChild, 'partsX');
			if(isNaN(plane_partsX))
			{
				return "Error at the primitive(plane) element with id = " + id + ". The partsX value is not a number.";
			}
			var plane_partsY = this.reader.getInteger(primitiveChild, 'partsY');
			if(isNaN(plane_partsY))
			{
				return "Error at the primitive(plane) element with id = " + id + ". The partsY value is not a number.";
			}
			primitiveData = new PlaneData(id, plane_dimX, plane_dimY, plane_partsX, plane_partsY);
		}
		else if(primitiveChild.nodeName == 'patch')
		{
			var patch_orderU = this.reader.getFloat(primitiveChild, 'orderU');
			if(isNaN(patch_orderU))
			{
				return "Error at the primitive(patch) element with id = " + id + ". The orderU value is not a number.";
			}
			var patch_orderV = this.reader.getFloat(primitiveChild, 'orderV');
			if(isNaN(patch_orderV))
			{
				return "Error at the primitive(patch) element with id = " + id + ". The orderV value is not a number.";
			}
			var patch_partsU = this.reader.getInteger(primitiveChild, 'partsU');
			if(isNaN(patch_partsU))
			{
				return "Error at the primitive(patch) element with id = " + id + ". The partsU value is not a number.";
			}
			var patch_partsV = this.reader.getInteger(primitiveChild, 'partsV');
			if(isNaN(patch_partsV))
			{
				return "Error at the primitive(patch) element with id = " + id + ". The partsV value is not a number.";
			}
			primitiveData = new PatchData(id, patch_orderU, patch_orderV, patch_partsU, patch_partsV);
			
			//Parse all the sub-elements of the patch
			var primitiveChild_nnodes = primitiveChild.children.length;
			
			var controlPoints_n = (patch_orderU + 1) * (patch_orderV + 1);
			
			if(primitiveChild_nnodes < 1)
			{
				return "Primitive(patch) with id = " + id + " does not have at least one controlpoint element. Number of elements found: " + primitiveChild_nnodes + ".";
			}
			
			if(primitiveChild_nnodes != controlPoints_n)
			{
				return "Primitive(patch) with id = " + id + " does not have the right amount of controlpoint elements. Number of elements found: " + primitiveChild_nnodes + ". The number of elements should be: " + controlPoints_n + ".";
			}
			
			for(var j = 0; j < primitiveChild_nnodes; j++)
			{
				var aControlPoint = primitiveChild.children[j];
				
				if(aControlPoint.nodeName != 'controlpoint')
				{
					return "Unknown 'child' element found at the 'patch' primitive element with id = " + id + ", with the tag: " + aControlPoint.nodeName + ".";
				}
				
				var controlPointX = this.reader.getFloat(aControlPoint, 'x');
				if(isNaN(controlPointX))
				{
					return "Error at the 'patch' primitive element with id = " + id + ". The x value is not a number. Controlpoint index: " + j + ".";
				}
				
				var controlPointY = this.reader.getFloat(aControlPoint, 'y');
				if(isNaN(controlPointY))
				{
					return "Error at the 'patch' primitive element with id = " + id + ". The y value is not a number. Controlpoint index: " + j + ".";
				}
				
				var controlPointZ = this.reader.getFloat(aControlPoint, 'z');
				if(isNaN(controlPointZ))
				{
					return "Error at the 'patch' primitive element with id = " + id + ". The z value is not a number. Controlpoint index: " + j + ".";
				}
				
				primitiveData.addControlPoint(controlPointX,controlPointY,controlPointZ);
			}
			
		}
		else if(primitiveChild.nodeName == 'vehicle')
		{
			primitiveData = new VehicleData(id);
		}
		else if(primitiveChild.nodeName == 'chessboard')
		{
			var chessboard_dU = this.reader.getInteger(primitiveChild, 'du');
			if(isNaN(chessboard_dU))
			{
				return "Error at the primitive(chessboard) element with id = " + id + ". The du value is not a number.";
			}
			var chessboard_dV = this.reader.getInteger(primitiveChild, 'dv');
			if(isNaN(chessboard_dV))
			{
				return "Error at the primitive(chessboard) element with id = " + id + ". The dv value is not a number.";
			}

			var chessboard_textureref = this.reader.getString(primitiveChild, 'textureref');
			
			if(!(chessboard_textureref in this.texturesMap))
			{
				return "Error at the primitive(chessboard) element with id = " + id + ". Unknown texture ref found: " + chessboard_textureref + ".";			
			}

			this.texturesMap[chessboard_textureref].setAsLoaded();
			var chessboard_texture = this.texturesMap[chessboard_textureref];

			var chessboard_sU = this.reader.getInteger(primitiveChild, 'su');
			if(isNaN(chessboard_sU))
			{
				return "Error at the primitive(chessboard) element with id = " + id + ". The su value is not a number.";
			}
			var chessboard_sV = this.reader.getInteger(primitiveChild, 'sv');
			if(isNaN(chessboard_sV))
			{
				return "Error at the primitive(chessboard) element with id = " + id + ". The sv value is not a number.";
			}

			primitiveData = new ChessboardData(id, chessboard_dU, chessboard_dV, chessboard_texture, chessboard_sU, chessboard_sV);
						
			//Parse all the sub-elements of the chessboard
			var primitiveChild_nnodes = primitiveChild.children.length;
			
			if(primitiveChild_nnodes != 3)
			{
				return "Primitive(chessboard) element with id = " + id + " does not have exactly three children. Number of elements found: " + primitiveChild_nnodes + ".";
			}
			
			for(var j = 0; j < primitiveChild_nnodes; j++)
			{
				var aColor = primitiveChild.children[j];
			
				if(!((aColor.nodeName == 'c1')||(aColor.nodeName == 'c2')|| (aColor.nodeName == 'cs')))
				{
					return "Unknown 'child' element found at the 'chessboard' primitive element with id = " + id + ", with the tag: " + aColor.nodeName + ".";
				}
				
				var r = this.reader.getFloat(aColor, 'r');
				if(isNaN(r))
				{
					return "Error at the 'chessboard' primitive element with id = " + id + ". The r value is not a number. Color index: " + j + ".";
				}
				var g = this.reader.getFloat(aColor, 'g');
				if(isNaN(g))
				{
					return "Error at the 'chessboard' primitive element with id = " + id + ". The g value is not a number. Color index: " + j + ".";
				}
				var b = this.reader.getFloat(aColor, 'b');
				if(isNaN(b))
				{
					return "Error at the 'chessboard' primitive element with id = " + id + ". The b value is not a number. Color index: " + j + ".";
				}
				var a = this.reader.getFloat(aColor, 'a');
				if(isNaN(a))
				{
					return "Error at the 'chessboard' primitive element with id = " + id + ". The a value is not a number. Color index: " + j + ".";
				}
				
				var rgba = new RGBAData(r,g,b,a);
				
				if(aColor.nodeName == 'c1')
				{
					primitiveData.setC1RGBA(rgba);
				}
				else if(aColor.nodeName == 'c2')
				{
					primitiveData.setC2RGBA(rgba);
				}
				else if(aColor.nodeName == 'cs')
				{
					primitiveData.setCSRGBA(rgba);
				}	
			}
		}
		else
...
MySceneGraph.prototype.parseComponents= function(componentsElem) 
{
	...
	for(var i = 0; i < components_nnodes; i++)
	{
		...
		if(this.componentsMap.isSpecialComponentName(id))
		{
			return "Component may not be defined with the special id " + id + ".";
		}
		...
		{
			return "The component element with id = " + id + " does not have at least four children elements: 'transformation', 'materials', 'texture', 'children' (and 'animationref' elements). Number of elements found: " + component_nnodes + ".";			
		}
		...
		//Parse the animations element
		var animationElems = aComponent.getElementsByTagName('animation');
		
		if (animationElems.length > 0)
		{
			errorMsg = this.testUniqueElement(animationElems,'animation','component');
			if(errorMsg != null)
			{
				return "Error at the component element with id = " + id + ". " + errorMsg;
			}

			var animationElem = animationElems[0];
			
			var tween = this.reader.getBoolean(animationElem, 'tween');
			
			if(tween) //key_frame_animation
			{
				var animation_nnodes=animationElem.children.length;
				
				if(animation_nnodes != 1)
				{
					return "Error at the component element with id = " + id + ". " + "Animations element with tween = true does not have exactly one child.";
				}
				
				var animationRefElem = animationElem.children[0];
				
				if(animationRefElem.nodeName != "animationref")
				{
					return "Error at the component element with id = " + id + ". Animation element has a child with an invalid tag: " + animationRefElem.nodeName + ".";			
				}
				
				var animationId = this.reader.getString(animationRefElem, 'id');

				if(!(animationId in this.keyFrameAnimationsMap))
				{
					return "Error at the component element with id = " + id + ". Unknown key frame animation id found: " + animationId + ".";			
				}

				this.keyFrameAnimationsMap[animationId].setAsLoaded();
				componentData.setKeyFrameAnimation(this.keyFrameAnimationsMap[animationId]);
			}
			else //other types of animations
			{
				var loop = this.reader.getBoolean(animationElem, 'loop');

				if(loop)
				{
					componentData.setAnimationsLoop(true);
				}

				var animation_nnodes=animationElem.children.length;
				for(let i = 0; i < animation_nnodes; i++)
				{
					var animationRefElem = animationElem.children[i];
					...
					if(!(animationId in this.animationsMap))
					{
						return "Error at the component element with id = " + id + ". Unknown animation id found: " + animationId + ".";			
					}

					this.animationsMap[animationId].setAsLoaded();
					componentData.addAnimation(this.animationsMap[animationId]);
				}
			}
		}
...
/*
 * Parses the 'animations' element.
 */
MySceneGraph.prototype.parseAnimations= function(animationsElem) 
{
	var animations_nnodes=animationsElem.children.length;

	this.animationsMap = {};
	this.keyFrameAnimationsMap = {};
	
	var errorMsg;
	
	for(var i = 0; i < animations_nnodes; i++)
	{
		var anAnimation = animationsElem.children[i];

		if(anAnimation.nodeName != 'animation')
		{
			return "Unknown 'child' element found at the 'animations' element, with the tag: " + anAnimation.nodeName + ".";
		}

		var id = this.reader.getString(anAnimation, 'id');
		
		var type = this.reader.getString(anAnimation, 'type');

		if(type == 'key_frame')
		{
			if(id in this.keyFrameAnimationsMap)
			{
				return "Two key_frame animations from the 'animations' element have identical id's. Repeated id: " + id + ".";
			}
			
			var animData = new KeyFrameAnimation(id,1.0);

			var animation_nnodes=anAnimation.children.length;
			if(animation_nnodes < 1)
			{
				return "Animation element with id = " + id + " does not have at least one 'controlpoint' children elements.";
			}
			
			for(var j = 0; j < animation_nnodes; j++)
			{
				var aControlPoint = anAnimation.children[j];

				if(aControlPoint.nodeName != 'controlpoint')
				{
					return "Unknown 'child' element found at the 'animation' element with id = " + id + ", with the tag: " + aControlPoint.nodeName + ".";
				}
				
				var t = this.reader.getFloat(aControlPoint, 't');
				if(isNaN(t))
				{
					return "Error at the animation element with id = " + id + ". The t value is not a number. Controlpoint index: " + j + ".";
				}
				
				var translate_x = this.reader.getFloat(aControlPoint, 'translate_x');
				if(isNaN(translate_x))
				{
					return "Error at the animation element with id = " + id + ". The translate_x value is not a number. Controlpoint index: " + j + ".";
				}
				
				var translate_y = this.reader.getFloat(aControlPoint, 'translate_y');
				if(isNaN(translate_y))
				{
					return "Error at the animation element with id = " + id + ". The translate_y value is not a number. Controlpoint index: " + j + ".";
				}
				
				var translate_z = this.reader.getFloat(aControlPoint, 'translate_z');
				if(isNaN(translate_z))
				{
					return "Error at the animation element with id = " + id + ". The translate_z value is not a number. Controlpoint index: " + j + ".";
				}
				
				var rotate_x = this.reader.getFloat(aControlPoint, 'rotate_x');
				if(isNaN(rotate_x))
				{
					return "Error at the animation element with id = " + id + ". The rotate_x value is not a number. Controlpoint index: " + j + ".";
				}
				
				var rotate_y = this.reader.getFloat(aControlPoint, 'rotate_y');
				if(isNaN(rotate_y))
				{
					return "Error at the animation element with id = " + id + ". The rotate_y value is not a number. Controlpoint index: " + j + ".";
				}
				
				var rotate_z = this.reader.getFloat(aControlPoint, 'rotate_z');
				if(isNaN(rotate_z))
				{
					return "Error at the animation element with id = " + id + ". The rotate_z value is not a number. Controlpoint index: " + j + ".";
				}
				
				var scale_x = this.reader.getFloat(aControlPoint, 'scale_x');
				if(isNaN(scale_x))
				{
					return "Error at the animation element with id = " + id + ". The scale_x value is not a number. Controlpoint index: " + j + ".";
				}
				
				var scale_y = this.reader.getFloat(aControlPoint, 'scale_y');
				if(isNaN(scale_y))
				{
					return "Error at the animation element with id = " + id + ". The scale_y value is not a number. Controlpoint index: " + j + ".";
				}
				
				var scale_z = this.reader.getFloat(aControlPoint, 'scale_z');
				if(isNaN(scale_z))
				{
					return "Error at the animation element with id = " + id + ". The scale_z value is not a number. Controlpoint index: " + j + ".";
				}
				
				errorMsg = animData.addControlPoint(t,translate_x,translate_y,translate_z,rotate_x,rotate_y,rotate_z,scale_x,scale_y,scale_z);
				if(errorMsg != null)
				{
					return "Error at the animation element with id = " + id + ". " + errorMsg;
				};
			}

			this.keyFrameAnimationsMap[id] = animData;
		}
		else //non-key_frame animation
		{
			if(id in this.animationsMap)
			{
				return "Two animations from the 'animations' element have identical id's. Repeated id: " + id + ".";
			}

			var span = this.reader.getFloat(anAnimation, 'span');
			if(isNaN(span))
			{
				return "Error at the animation element with id = " + id + ". The span value is not a number.";
			}

			if(type == 'linear')
			{
				var animData = new LinearAnimation(id,span);

				var animation_nnodes=anAnimation.children.length;
				if(animation_nnodes < 2)
				{
					return "Animation element with id = " + id + " does not have at least two 'controlpoint' children elements.";
				}
				...
				for(var j = 0; j < animation_nnodes; j++)
				{
					...
					var xx = this.reader.getFloat(aControlPoint, 'xx');
					if(isNaN(xx))
					{
						return "Error at the animation element with id = " + id + ". The xx value is not a number. Controlpoint index: " + j + ".";
					}
					
					var yy = this.reader.getFloat(aControlPoint, 'yy');
					if(isNaN(yy))
					{
						return "Error at the animation element with id = " + id + ". The yy value is not a number. Controlpoint index: " + j + ".";
					}
					
					var zz = this.reader.getFloat(aControlPoint, 'zz');
					if(isNaN(zz))
					{
						return "Error at the animation element with id = " + id + ". The zz value is not a number. Controlpoint index: " + j + ".";
					}
					
					animData.addControlPoint(xx,yy,zz);
				}
			}
			else if(type == 'circular')
			{
				var centerx = this.reader.getFloat(anAnimation, 'centerx', true);
				if(isNaN(centerx))
				{
					return "Error at the animation element with id = " + id + ". The centerx value is not a number.";
				}
				var centery = this.reader.getFloat(anAnimation, 'centery', true);
				if(isNaN(centery))
				{
					return "Error at the animation element with id = " + id + ". The centery value is not a number.";
				}
				var centerz = this.reader.getFloat(anAnimation, 'centerz', true);
				if(isNaN(centerz))
				{
					return "Error at the animation element with id = " + id + ". The centerz value is not a number.";
				}
				
				var radius = this.reader.getFloat(anAnimation, 'radius');
				if(isNaN(radius))
				{
					return "Error at the animation element with id = " + id + ". The radius value is not a number.";
				}
				
				var startang = this.reader.getFloat(anAnimation, 'startang');
				if(isNaN(startang))
				{
					return "Error at the animation element with id = " + id + ". The startang value is not a number.";
				}
				
				var rotang = this.reader.getFloat(anAnimation, 'rotang');
				if(isNaN(rotang))
				{
					return "Error at the animation element with id = " + id + ". The rotang value is not a number.";
				}
				
				var animData = new CircularAnimation(id,span,centerx,centery,centerz,radius,startang,rotang);
			}
			else if(type == 'elliptical')
			{
				...
				var radiusZ = this.reader.getFloat(anAnimation, 'radiusZ');
				if(isNaN(radiusZ))
				{
					return "Error at the animation element with id = " + id + ". The radiusZ value is not a number.";
				}
				
				var radiusX = this.reader.getFloat(anAnimation, 'radiusX');
				if(isNaN(radiusX))
				{
					return "Error at the animation element with id = " + id + ". The radiusX value is not a number.";
				}
				...
				var animData = new EllipticalAnimation(id,span,centerx,centery,centerz,radiusZ,radiusX,startang,rotang);
			}
			else
			{
				return "Error at the animation element with id = " + id + ". The animation type is not one of the following: 'linear', 'circular', 'elliptical', 'key_frame'.";
			}

			this.animationsMap[id] = animData;
		}

		errorMsg = animData.testParams();
		if(errorMsg != null)
		{
			return errorMsg;
		}
...
/*
 * Parses the 'viewpoints' element.
 */
MySceneGraph.prototype.parseViewpoints= function(viewpointsElem) 
{
	var viewpoints_nnodes=viewpointsElem.children.length;

	this.viewpointsMap = {};
	for(var i = 0; i < viewpoints_nnodes; i++)
	{
		var aViewpoint = viewpointsElem.children[i];

		if(aViewpoint.nodeName != 'viewpoint')
		{
			return "Unknown 'child' element found at the 'viewpoints' element, with the tag: " + aViewpoint.nodeName + ".";
		}

		var id = this.reader.getString(aViewpoint, 'id');

		if(id in this.viewpointsMap)
		{
			return "Two viewpoints from the 'viewpoints' element have identical id's. Repeated id: " + id + ".";
		}

		//Parse 'from' and 'to' elements
		var viewpoint_nnodes=aViewpoint.children.length;
		if(viewpoint_nnodes != 2)
		{
			return "Viewpoint element does not have exactly two children elements: 'from' and 'to'.";
		}

		//Parse the from element
		var fromElems = aViewpoint.getElementsByTagName('from');
		
		var errorMsg = this.testUniqueElement(fromElems,'from','viewpoint');
		if(errorMsg != null)
		{
			return "Error at the viewpoint element with id = " + id + ". " + errorMsg;
		}
		...
		if(isNaN(fromX))
		{
			return "Error at the viewpoint element with id = " + id + ". " + "The x value of the 'from' element is not a number.";
		}
		var fromY = this.reader.getFloat(fromElem, 'y');
		if(isNaN(fromY))
		{
			return "Error at the viewpoint element with id = " + id + ". " + "The y value of the 'from' element is not a number.";
		}
		var fromZ = this.reader.getFloat(fromElem, 'z');
		if(isNaN(fromZ))
		{
			return "Error at the viewpoint element with id = " + id + ". " + "The z value of the 'from' element is not a number.";
		}

		//Parse the to element
		var toElems = aViewpoint.getElementsByTagName('to');
		
		errorMsg = this.testUniqueElement(toElems,'to','viewpoint');
		if(errorMsg != null)
		{
			return "Error at the viewpoint element with id = " + id + ". " + errorMsg;
		}
		...
		if(isNaN(toX))
		{
			return "Error at the viewpoint element with id = " + id + ". " + "The x value of the 'to' element is not a number.";
		}
		var toY = this.reader.getFloat(toElem, 'y');
		if(isNaN(toY))
		{
			return "Error at the viewpoint element with id = " + id + ". " + "The y value of the 'to' element is not a number.";
		}
		var toZ = this.reader.getFloat(toElem, 'z');
		if(isNaN(toZ))
		{
			return "Error at the viewpoint element with id = " + id + ". " + "The z value of the 'to' element is not a number.";
		}

		//Store all the data in an object
		var viewpointData = new Viewpoint(id, fromX, fromY, fromZ, toX, toY, toZ);
	
		this.viewpointsMap[id] = viewpointData;
	}
...
/*
 * Gets the viewpointsMap field.
 */
MySceneGraph.prototype.getViewpointsMap= function() 
{	
	return this.viewpointsMap;
}